...
class Attentive_pool(nn.Module):
    ...
    def forward(self, feat):
        ...
        weight = F.softmax(weight,dim=-1)     #(2,128,2048,64)
        new_feat = torch.sum(feat*weight,dim=-1, keepdim=True)
...
class PointnetSAModuleVotes(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Linear(mlp[-1] * 2, mlp[-1], bias=False)
        self.bn1 = nn.BatchNorm1d(mlp[-1])
        self.attpool = Attentive_pool(mlp[-1])
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        new_features2 = self.attpool(new_features)  #(B,c,n,1)
        x_max = F.max_pool2d(new_features, kernel_size=[1, new_features.size(3)])  # (B, mlp[-1], npoint, 1)
        x = torch.cat((x_max, new_features2), dim=1).squeeze(-1)
